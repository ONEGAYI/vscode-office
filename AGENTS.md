...
- fork PR #14（feat #11+#12）：ir list marker 编辑——#11 探针 GO（报告见 issue 评论，推翻"data-marker 空串"旧观察）+ #12 双模式落地（Lute 门 8 方法/liftOutOfList 列表外落点/CSS scope 扩展 .vditor-ir，IL0–IL6 + IE1–IE6 契约），review-loops 收敛；追加 25f6838/606d2fa 修复用户实测空项系列问题：空 li 零子节点，浏览器会把 li 内任何 caret 规范化进唯一子节点 span 文本（键入被 input gate 吞、Backspace 逐字符删 marker）——最终方案是**空内容项不激活 live span**（hasOwnContent 判定），vditor 原生键入/删项路径保留；非空项行首点击仍迁移 caret 到 span 后。教训：jsdom 不做 caret 规范化，span 相关契约须 dev 宿主真机复核
